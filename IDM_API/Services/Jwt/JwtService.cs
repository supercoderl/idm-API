...
namespace IDM_API.Services.Jwt
{
	public class JwtService : IJwtService
	{
		...
		public JwtService(IConfiguration configuration, IDMContext context)
        {
			_configuration = configuration;
			_context = context;
		}

        public LoginResult GenerateToken(tbl_user user, List<Claim> claims, DateTime now)
		{
			...
			_context.tbl_tokens.Add(newToken);
		...
		{
			var refreshTokenObject = await _context.tbl_tokens.SingleAsync(x => x.Token == refreshToken);
			var user = await _context.tbl_users.FindAsync(refreshTokenObject.UserID);
		...
		{
			var item = await _context.tbl_tokens.FirstOrDefaultAsync(x => x.Token == token);
			...
			_context.tbl_tokens.Entry(item).State = EntityState.Modified;
